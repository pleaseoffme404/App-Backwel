...
export function CatalogLinkSection({ config, isLoading }: CatalogLinkSectionProps) {
  ...
  const bgImage = config?.banner_image_url || 'https://images.unsplash.com/photo-1555680202-c86f0e12f086?q=80&w=2070';
  const bannerText = config?.banner_text || '¿Listo para transformar tu setup?';
  const buttonText = config?.button_text || 'Explorar Catálogo';
