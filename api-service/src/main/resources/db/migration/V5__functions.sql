...
CREATE OR REPLACE FUNCTION fn_process_category_discount(
    category_path UUID[]
) RETURNS NUMERIC(5,4)
    LANGUAGE plpgsql
AS $$
...
BEGIN
    WITH
        ...
        ranked_discounts AS (
            SELECT
                ...
                decimal_value,
                ROW_NUMBER() OVER (PARTITION BY stackable ORDER BY decimal_value DESC) AS rn
            FROM target_discounts
        ),
        all_strategies AS (
            -- Estrategia 1: Sumar hasta los 3 mejores descuentos acumulables
            SELECT
                COALESCE(SUM(decimal_value), 0) AS potential_discount
            FROM ranked_discounts
            WHERE stackable = true AND rn <= 3
            ...
            -- Estrategia 2: Tomar el descuento no acumulable más alto
            SELECT
                COALESCE(SUM(decimal_value), 0) AS potential_discount
            FROM ranked_discounts
            WHERE stackable = false AND rn = 1
    ...
    -- Retornamos el máximo valor entre ambas estrategias
    SELECT
        CAST(ROUND(MAX(potential_discount), 4) AS NUMERIC(5,4))
    INTO v_discount_decimal
    FROM all_strategies;
